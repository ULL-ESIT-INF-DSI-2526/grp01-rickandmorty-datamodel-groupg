...
export class CLI {
  ...
  constructor(private readonly db: DbManager, private readonly search: SearchEngine, private readonly manager: MultiverseManager) {
    this.handlers = {
  ...
  public async run(): Promise<void> {
    ...
    while (!exit) {
      ...
      if (choice === 'exit' || !choice) exit = true;
      else await this.handleSelection(choice);
    }
  }
  ...
  private async crudMenu(menu: EntityMenu): Promise<void> {
    ...
    try {
      switch (op) {
        ...
        case 'add':
          ...
          break;
        case 'mod':
          const { id } = await prompts({ type: 'text', name: 'id', message: 'Enter ID to modify:' });
          await handler.handleModify(id);
          await this.db.save();
          console.log("Update successful.");
          break;
        case 'del':
          const { delId } = await prompts({ type: 'text', name: 'delId', message: 'Enter ID to remove:' });
          this.db[menu].remove(delId);
          await this.db.save();
    ...
    if (op && op !== 'back') await prompts({ type: 'text', name: 'p', message: 'Press enter to continue...' });
  }
